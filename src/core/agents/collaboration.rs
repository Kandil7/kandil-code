...
impl CollaborationManager {
    ...
    pub async fn start_collaboration_session(&mut self, title: &str, participants: Vec<Role>, agenda: Vec<String>) -> String {
    ...
    pub async fn facilitate_architecture_discussion(
        ...
        let arch_input = architect_agent.ai.chat(&format!("Discuss architecture for: {}", topic)).await?;
        discussion.push(format!("Architect: {}", arch_input));
        
        // Developer's perspective
        let dev_input = developer_agent.ai.chat(&format!("From development perspective, consider: {}", topic)).await?;
        discussion.push(format!("Developer: {}", dev_input));
        
        // QA's perspective
        let qa_input = qa_agent.ai.chat(&format!("From QA perspective, consider: {}", topic)).await?;
        discussion.push(format!("QA: {}", qa_input));
        
        // Store the discussion
        self.ongoing_discussions.insert(topic.to_string(), discussion.clone());
    ...
    pub async fn create_cross_role_decision(
        ...
        for role in &involved_roles {
            match role {
                Role::Architect => {
                    let input = architect_agent.ai.chat(&format!("From architecture perspective, consider: {}", description)).await?;
                ...
                Role::Developer => {
                    let input = developer_agent.ai.chat(&format!("From development perspective, consider: {}", description)).await?;
                    discussion_points.push(DiscussionPoint {
                        role: role.clone(),
                        point: input,
                        concerns: vec![],
                ...
                Role::QA => {
                    let input = qa_agent.ai.chat(&format!("From QA perspective, consider: {}", description)).await?;
                    discussion_points.push(DiscussionPoint {
                        role: role.clone(),
                        point: input,
                        concerns: vec![],
        ...
        let final_decision = format!("Decision synthesized from inputs from: {:?}", involved_roles);
